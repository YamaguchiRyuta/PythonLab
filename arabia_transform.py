...
class Kansuji2Arabic(object):
    ...
    def read_arabia_configs(self):
        default_setting = {
            ...
            "単位": "月分日時秒円番第台銭年週行桁件個字株人票通話十百千億万兆点回"
        }
        self.setting = default_setting

        config_filename = "arabia_configs.json"
        arabia_configs_path = os.path.join(os.environ['ALLUSERSPROFILE'], "tr-manager", config_filename)

        # 設定ファイルが存在しない場合は初期値で作成
        if not os.path.isfile(arabia_configs_path):
            self.make_arabia_configs(arabia_configs_path, default_setting)
            logger.info("アラビア変換設定ファイルが見つからなかったため、初期値で作成しました): "
                        + arabia_configs_path)
    ...
    def make_arabia_configs(self, arabia_configs_path, default_setting):
        # C:\\ProgramData\\tr-managerフォルダが無ければ作成
        if not os.path.isdir(os.path.dirname(arabia_configs_path)):
            os.mkdir(os.path.dirname(arabia_configs_path))
        # C:\\ProgramData\\tr-manager\\arabia_configs.jsonを作成
        with open(arabia_configs_path, mode="x", encoding='utf_8') as arabia_config:
            arabia_config.write(json.dumps(default_setting, indent=4, ensure_ascii=False))
    ...
    def execute(self, lattice_obj, addwords, force_trans=False):
        ...
        for self.speaker, lattices in lattice_obj.items():
            ...
            for current_id, current_data in sorted(lattices.items(),
                ...
                if current_word in ["!NULL", "!ENTER", "!EXIT", "!LG_HANGUP", "!SILENCE", "!PAUSE-FILLER"]:
    ...
    def arabic2kansuji(self):
        ...
        for speaker, lattices in self.return_lattice.items():
            ...
            for current_id, current_data in lattices.items():
                if current_data["word"] not in ["!NULL", "!ENTER", "!EXIT", "!LG_HANGUP", "!SILENCE", "!PAUSE-FILLER"] and current_data["word"].replace(" ", "") != "":
        ...
        for speaker, lattices in self.return_lattice.items():
            ...
            for i, (current_id, current_data) in enumerate(
                ...
                if current_word in self.omit_list2:
                    ...
                    elif current_word in ["!NULL", "!ENTER", "!EXIT", "!LG_HANGUP", "!SILENCE", "!PAUSE-FILLER"]:
                        # IDを退避
    ...
    def trans_omit(self, arabic2kansuji_ids, speaker):
        ...
        for reverse_id in arabic2kansuji_ids:
            ...
            if len(words) == 0 or words in ["!NULL", "!ENTER", "!EXIT", "!LG_HANGUP", "!SILENCE", "!PAUSE-FILLER"]:
                continue
            temp = ""
            # 区切った単語を順に読み込む
        ...
        for speaker, lattices in self.return_lattice.items():
            ...
            for current_id, current_data in sorted(lattices.items(),
                ...
                for i, word in enumerate(current_data["word"].split()):
                    if word in ["!NULL", "!ENTER", "!EXIT", "!LG_HANGUP", "!SILENCE", "!PAUSE-FILLER"]:
                        # 2秒未満の!NULL !ENTER !EXITは無視するようにしていたが、時間に関わらず無視してよい
                        # とのことで修正
                        # word_time_length = float(current_data["end"]) - float(current_data["start"])
                        # if word_time_length >= 2:
                ...
                if (not break_word and current_id not in update_id
                        and current_data["word"] not in ["!NULL", "!ENTER", "!EXIT", "!LG_HANGUP", "!SILENCE", "!PAUSE-FILLER"]
